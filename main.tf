...
locals {
  repository_environment_map = { for id, e in var.environment : try(e._key, e.name) => id }
}
...
resource "github_repository_environment" "repository_environment" {
  ...
  environment = var.environment[each.value].name
  repository  = github_repository.repository.name
  wait_timer  = try(var.environment[each.value].wait_timer, 0)
  reviewers {
    users = try(var.environment[each.value].reviewers.users, [])
    teams = try(var.environment[each.value].reviewers.teams, [])
  }
  deployment_branch_policy {
    protected_branches     = var.environment[each.value].deployment_branch_policy.protected_branches
    custom_branch_policies = var.environment[each.value].deployment_branch_policy.custom_branch_policies
  }
}
